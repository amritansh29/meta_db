...
const StudySeries = () => {
  ...
  // Query for series of this study with pagination, filters, and sorting
  const { data: seriesData, isLoading, error, refetch } = useQuery({
    queryKey: ['study-series', studyId, page, filters, { sort: sortConfig }],
    queryFn: () => seriesApi.getAllSeries({
      limit: PAGE_SIZE,
      skip: (page - 1) * PAGE_SIZE,
      filters: {
        ...filters,
        studyId: studyId, // Add study filter
      },
      sort: getMongoSort(),
    }),
    enabled: !!studyId,
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold">Study Series</h1>
          <p className="text-gray-600 mt-1">Browse all series for this study</p>
      ...
      <div className="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        {series.length === 0 && (
          <div className="px-6 py-8 text-center text-gray-500">
            No series found for this study matching the current filters
          </div>
        )}
      </div>
